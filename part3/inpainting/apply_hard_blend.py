...
def load_mask_png(path: Path, feather: int = 0) -> np.ndarray:
    ...
    DAVIS Annotations use palette values 38/75 for foreground (not 255),
    so we threshold at >0, not >127.
    mask_frames/ (from prepare_diffueraser_inputs.py) already use 0/255.
    """
    img = cv2.imread(str(path), cv2.IMREAD_GRAYSCALE)
    if img is None:
        raise FileNotFoundError(f"Cannot read mask: {path}")
    binary = (img > 0).astype(np.float32)  # fixed: was >127, breaks DAVIS 38/75 annotations
...
def main():
    ...
    # Prefer mask_frames/ from base_dir: already 0/255 binary with proper dilation
    # (generated by prepare_diffueraser_inputs.py using >0 threshold on DAVIS annotations)
    # Fall back to DAVIS GT masks only if mask_frames is missing.
    mask_frames_dir = base_dir / "mask_frames"
    if mask_frames_dir.exists():
        mask_files = sorted(mask_frames_dir.glob("*.png"))
        print(f"Using mask_frames from base version: {mask_frames_dir} ({len(mask_files)} files)")
    else:
        gt_mask_dir = DAVIS_GT_MASKS / args.sequence
        if gt_mask_dir.exists():
            mask_files = sorted(gt_mask_dir.glob("*.png"))
            print(f"Fallback: Using DAVIS GT mask dir: {gt_mask_dir} ({len(mask_files)} masks)")
        else:
            print("ERROR: no mask source found", file=sys.stderr)
            sys.exit(1)
    if len(mask_files) == 0:

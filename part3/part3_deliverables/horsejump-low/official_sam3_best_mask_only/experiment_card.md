...
| motivation | 看它相对单 prompt 是否只在少数序列改善，还是整体都提升。 |
| expected_gain | 见 `plain_explanation` |
| actual_result | 它说明 prompt 确实重要，但仅靠 prompt 调整还不够。 |
| next_decision | 待实验结果更新 |

## 关键路径

- `mask_frames`: `/data3/jli657/project3/part3/outputs/official_sam3_best/masks/horsejump-low`

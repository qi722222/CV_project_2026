...
- 这个素材难点：不仅要去掉车，还要处理地面阴影；阴影是否算目标会直接影响分数。
...
- `direction_a_shadow_geom_scale_sweep_mask_only` | mask_only | failed_or_exploratory | Direction A car-shadow 阴影几何先验扫参
  这条方法一句话：这是专门给 car-shadow 做的阴影扩展先验，不是通用方法。
